    ...
    <div className="min-h-screen flex flex-col items-center justify-center px-4 py-16 relative">
      ...
      <div className="grid sm:grid-cols-2 gap-6 w-full max-w-2xl">
        ...
        <button
          ...
          onMouseLeave={() => setHovered(null)}
          className="group relative rounded-3xl glass-strong p-8 text-left border border-white/10 hover:border-cyan-400/40 hover:ring-glow transition-all duration-300 hover:scale-[1.02] cursor-pointer overflow-hidden"
        >
          ...
          <div className="relative z-10">
            <div className="h-14 w-14 rounded-2xl bg-gradient-to-br from-cyan-500 to-teal-600 flex items-center justify-center mb-5 shadow-[0_0_30px_rgba(34,211,238,0.4)] group-hover:shadow-[0_0_40px_rgba(34,211,238,0.6)] transition-shadow">
              <Mic className="h-7 w-7 text-white" />
            </div>
            <h3 className="text-xl font-semibold text-white mb-2">Use Voice Agent</h3>
            <p className="text-sm text-muted-foreground leading-relaxed">
              Tell our AI assistant about your interview needs. Just speak
              naturally — it'll handle the rest.
            </p>
            <div className="mt-6 flex flex-wrap gap-2">
              {["Hands-Free", "AI Powered", "Natural"].map((tag) => (
                <span key={tag} className="text-[11px] px-2.5 py-1 rounded-full bg-secondary/70 border border-white/5 text-muted-foreground">
                  {tag}
                </span>
              ))}
